...
export PTP_RUN_DIR="/tmp/ptp-run-$(date +%Y%m%d-%H%M%S)"
mkdir -p "${PTP_RUN_DIR}"
cleanup_run_dir() {
...
if [[ "$RUN_PHASE" == "images" ]]; then
    ...
    tar cf "${PTP_RUN_DIR}/ptp-images.tar" -C /tmp/ptp-images .
    echo "Images saved to ${PTP_RUN_DIR}/ptp-images.tar ($(du -h "${PTP_RUN_DIR}/ptp-images.tar" | cut -f1))"

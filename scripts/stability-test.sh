...
#                   tgm, tgmoc, tgmbc) or "all" for all modes sequentially
#
# Optional:
...
if [[ -z "${MODE}" ]]; then
  echo "Error: --mode is required (oc, bc, dualnicbc, dualnicbcha, dualfollower, tgm, tgmoc, tgmbc, or all)"
  exit 1
fi

if [[ "${MODE}" == "all" ]]; then
  MODES=(oc bc dualnicbc dualnicbcha dualfollower tgm tgmoc tgmbc)
else

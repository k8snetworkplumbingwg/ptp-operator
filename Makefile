...
$(KUSTOMIZE): $(LOCALBIN)
	...
	test -s $(LOCALBIN)/kustomize || { curl -Ss $(if $(GITHUB_TOKEN),-H "Authorization: token $(GITHUB_TOKEN)") $(KUSTOMIZE_INSTALL_SCRIPT) | bash -s -- $(subst v,,$(KUSTOMIZE_VERSION)) $(LOCALBIN); }

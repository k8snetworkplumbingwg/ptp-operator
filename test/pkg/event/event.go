...
// WaitForCloudEventProxyReady polls until the cloud-event-proxy process is
// running and its health endpoint returns OK. The pod reference is refreshed
// after container restart to avoid stale object errors.
func WaitForCloudEventProxyReady(pod *corev1.Pod) error {
	// Refresh the pod to pick up container restart status
	refreshPod := func() *corev1.Pod {
		updated, err := client.Client.CoreV1().Pods(pod.Namespace).Get(context.TODO(), pod.Name, metav1.GetOptions{})
		if err != nil {
			logrus.Debugf("could not refresh pod %s/%s: %v", pod.Namespace, pod.Name, err)
			return pod
		}
		return updated
	}
	...
	for time.Now().Before(deadline) {
		current := refreshPod()
		buf, _, err := pods.ExecCommand(client.Client, true, current, pkg.EventProxyContainerName,
			[]string{"sh", "-c", "pgrep -f ^./cloud-event-proxy"})
		if err == nil && strings.TrimSpace(buf.String()) != "" {
			logrus.Info("cloud-event-proxy process is running")
			processFound = true
			break
		}
		time.Sleep(2 * time.Second)
	...
	if !processFound {
		return fmt.Errorf("cloud-event-proxy process did not restart within 5 minutes")
	}
	...
	for time.Now().Before(deadline) {
		current := refreshPod()
		buf, _, err := pods.ExecCommand(client.Client, false, current, pkg.EventProxyContainerName,
	...
	if !healthOK {
		return fmt.Errorf("cloud-event-proxy health endpoint did not recover within 2 minutes")
	}
	return nil
}

...
func PtpConfigTelcoBC(isExtGM bool) error {
	...
	// using PTP hardware clock index and pin presence.
	wpcIfaces := ptphelper.GetWPCEnabledInterfaceNames(bc1SlaveIf.NodeName)
	IfList, firstNicInterface, deviceID, err := ptphelper.GetInterfacesByPHCAndPins(bc1SlaveIf.NodeName, wpcIfaces, bc1SlaveIf.IfName)
	if err != nil {
		return err
	}

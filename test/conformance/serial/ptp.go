...
- the node running the clock under test is label with: %s`, pkg.PtpClockUnderTestNodeLabel, pkg.PtpClockUnderTestNodeLabel)
		...
		Context("PTP ClockSync", func() {
			...
			It("Rogue PTP client without authentication cannot sync (injection test)", func() {
				...
				// Skip for BC/HA modes: The "rogue client" test verifies an unauthenticated slave
				// cannot sync with an authenticated master. In BC/HA topologies:
				// 1. Removing auth from downstream slave doesn't prevent it from receiving
				//    authenticated Sync messages from BC - it can still achieve LOCKED
				// 2. The discovery structures (DiscoveredClockUnderTestPod, DiscoveredFollowerInterfaces)
				//    point to BC, not to downstream slaves
				// 3. PTP auth primarily protects against rogue MASTERS, not rogue SLAVES
				// 4. In DualNICBCHA mode, redundant paths mean removing auth from one slave
				//    doesn't break sync - the other slave/BC path still works
				// The SPP mismatch and MITM tests adequately cover BC/HA auth validation.
				if fullConfig.PtpModeDesired == testconfig.BoundaryClock ||
					fullConfig.PtpModeDesired == testconfig.DualNICBoundaryClock ||
					fullConfig.PtpModeDesired == testconfig.DualNICBoundaryClockHA {
					Skip("Rogue client test not applicable to BC/HA mode - use SPP mismatch/MITM tests instead")
				}
			...
			It("Tampered PTP messages are dropped (MITM protection test)", func() {
				...
				// Determine which PtpConfig to modify:
				// - OC mode: Modify GM (test-grandmaster) - slave connects directly to GM
				// - BC mode: Modify BC (test-bc-master1) - slave connects to BC, not GM
				var targetPolicyName string
				if fullConfig.PtpModeDesired == testconfig.BoundaryClock ||
					fullConfig.PtpModeDesired == testconfig.DualNICBoundaryClock {
					targetPolicyName = pkg.PtpBcMaster1PolicyName
				} else {
					targetPolicyName = pkg.PtpGrandMasterPolicyName
				}
...
`,
				...
				By(fmt.Sprintf("Changing %s to use attacker keys (simulates compromised node)", targetPolicyName), func() {
					// Get target PtpConfig
					ptpConfig, err := client.Client.PtpConfigs(pkg.PtpLinuxDaemonNamespace).Get(
						context.Background(),
						targetPolicyName,
						metav1.GetOptions{},
					...
					modifiedConf := strings.Replace(ptp4lConf,
						...
						"sa_file /etc/ptp-secret-mount/ptp-security-attacker/ptp-security.conf", 1)
					ptpConfig.Spec.Profile[0].Ptp4lConf = &modifiedConf
					...
					fmt.Fprintf(GinkgoWriter, "%s now uses attacker keys (simulates MITM/compromised node)\n", targetPolicyName)
				...
				By(fmt.Sprintf("Restoring correct keys and sa_file to %s (cleanup)", targetPolicyName), func() {
					...
					restoredConf := strings.Replace(ptp4lConf,
						"sa_file /etc/ptp-secret-mount/ptp-security-attacker/ptp-security.conf",
						"sa_file /etc/ptp-secret-mount/ptp-security-conf/ptp-security.conf", 1)
					ptpConfig.Spec.Profile[0].Ptp4lConf = &restoredConf
					...
					// Wait for pods to fully restart and stabilize after restoration
					time.Sleep(60 * time.Second)
			...
			var _ = Context("Negative - run pmc in a new unprivileged pod on the slave node", func() {
				It("Should not be able to use the uds", func() {
					...
					Eventually(func() string {
						...
						return buf.String()
					}, 1*time.Minute, 2*time.Second).Should(ContainSubstring("Permission denied"), "unprivileged pod can access the q socket")
				})
			})

		...
		Context("WPC GM Events verification (V2)", Ordered, func() {
			...
			var _ = Describe("T-GM Event Tests", func() {
				It("should verify clock class change when GNSS is lost ", func() {
					...
					// Use the correct resource address format that matches the existing subscription format
					resourceAddress := event.FormatResourceAddress(event.ClockClassResourceAddress, fullNodeName)
					...
					logrus.Infof("ResourceAddress: %s", resourceAddress)
					...
					// STEP 1: Subscribe to clock class 6 events
					logrus.Info("=== STEP 1: Subscribing to clock class  events ===")
					subscriptionErr := event.SubscribeToEvent(publisherURL, resourceAddress, endpointURI)
					if subscriptionErr != nil {
						logrus.Errorf("Subscription failed: %v", subscriptionErr)
						Fail(fmt.Sprintf("Failed to subscribe to events: %v", subscriptionErr))
					}
					logrus.Info("SUCCESS: Subscription created successfully")
					...
					// STEP 2: Verify clock class 7 event (holdover state after cold boot)
					// Keep cold boot running while verifying clock class 7
